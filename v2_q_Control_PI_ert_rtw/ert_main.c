...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(v2_q_Control_PI_M) == (NULL)) &&
    !rtmGetStopRequested(v2_q_Control_PI_M);
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(v2_q_Control_PI_M) == (NULL)) &&
                      !rtmGetStopRequested(v2_q_Control_PI_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(v2_q_Control_PI_M, "stopping the model");
}
...
int app_main(int argc, char **argv)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(v2_q_Control_PI_M, 0);
  ...
  /* Initialize model */
  v2_q_Control_PI_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(v2_q_Control_PI_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (v2_q_Control_PI_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
    return errorCode;
  }
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(v2_q_Control_PI_M, true);
    }
  }
