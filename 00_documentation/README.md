...
| **Cable de Comunicación** | Cable USB a Micro-USB con líneas de datos | 1 | Programación y comunicación serial / *External Mode*. |
...
![Motor DC JGA25-371 con Encoder](./img/motor_jga25_hardware.png)
...
![Hoja de Datos Motor JGA25-371](./img/motor_jga25_datasheet.png)
...
![ESP32 WROOM 38 Pines](./img/esp32_wroom_38pin.png)

* **Voltaje lógico de operación:** 3.3 V
* **Frecuencia de reloj:** 240 MHz (Dual Core)
* **Entradas con interrupción externa:** Todos los pines GPIO admiten interrupciones por hardware (óptimo para lectura de señales de encoder en cuadratura).
...
![Driver L298N](./img/l298n_driver.png)
...
![Fuente Regulada 12V](./img/power_supply_12v.png)
...
![Diagrama Esquemático Eléctrico](./img/wiring_schematic_diagram.png)
...
| **ESP32** | GPIO 25 *(ejemplo)* | **L298N** | ENA | Señal PWM de modulación de velocidad |
| **ESP32** | GPIO 26 *(ejemplo)* | **L298N** | IN1 | Dirección de giro (Canal 1) |
| **ESP32** | GPIO 27 *(ejemplo)* | **L298N** | IN2 | Dirección de giro (Canal 2) |
...
| **Encoder Motor** | Canal A | **ESP32** | GPIO 18 *(ejemplo)* | Lectura de cuadratura (Interrupción) |
...
3. **Caída de Voltaje en el L298N:**  
   El L298N está basado en transistores bipolares (BJT), lo que genera una caída de voltaje interna aproximada de entre $1.8\text{ V}$ y $2.0\text{ V}$. Esto significa que con una fuente de $12\text{ V}$ y un ciclo de trabajo del $100\%$, el motor recibirá aproximadamente $10.0\text{ V} - 10.2\text{ V}$. Ten en cuenta esta limitación al calcular la ganancia estática $K$ durante la etapa de identificación.
4. **Desacoplo de Ruido:**  
